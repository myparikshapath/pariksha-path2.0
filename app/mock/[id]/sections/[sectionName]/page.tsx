    ...
    const loadCourseAndSectionData = useCallback(async () => {
        try {
            ...
            const [detailsResponse, questionsResponse] = await Promise.all([
                ...
                getSectionQuestions(courseData.id, sectionName, pagination.page, pagination.limit, undefined, undefined, "mock"),
            ]);
                ...
                <div className="space-y-2">
                    {question.options.map((option, idx) => (
                        <button
                            ...
                            className={`w-full text-left px-4 py-2 rounded border transition ${getOptionStyle(question, option.text)}`}
                        >
                            {option.text}
                        </button>
                    ))}
