...
export default function StudentsPage() {
  ...
  const viewStudentDetails = async (studentId: string) => {
    try {
      ...
      const err = error as AxiosError<{ message?: string }>;
      alert(err.response?.data?.message || err.message || "Failed to fetch student details");
    } finally {
      setLoading(false);
    }
  };
    ...
    <AdminRoute>
      ...
      <div className="w-full max-w-9xl bg-gradient-to-br from-green-100 to-emerald-50 rounded-xl shadow-lg border border-green-200/50 p-6 sm:p-8 mb-6">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
          <div>
            <h1 className="text-3xl sm:text-4xl font-bold text-green-900 mb-2 sm:mb-0">Students</h1>
            <p className="text-sm sm:text-lg text-green-800">Manage all registered students and their enrollments</p>
          </div>
          <Button
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 md:hidden">
            {students.map((s, idx) => (
              <motion.div key={s.id} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: idx * 0.05 }}
              ...
              >
                ...
                <div className="mt-3">
                  <span className={`px-3 py-1 text-xs font-semibold rounded-full ${s.is_active ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}>
                    {s.is_active ? "Active" : "Inactive"}
                  </span>
                </div>
                <div className="mt-4 flex flex-col gap-2">
                  <Button onClick={() => s.is_active ? deactivateStudent(s.id) : activateStudent(s.id)}
                    className={`w-full ${s.is_active ? "bg-red-500 hover:bg-red-600" : "bg-green-500 hover:bg-green-600"} text-white`}
                  >
                    {s.is_active ? "Deactivate" : "Activate"}
                  </Button>
                  <Button onClick={() => viewStudentDetails(s.id)} className="w-full bg-blue-500 hover:bg-blue-600 text-white">
                    View Details
                  </Button>
                </div>
              </motion.div>
          ...
          <div className="hidden md:block overflow-x-auto rounded-xl shadow-lg border border-gray-200 w-full">
            <table className="w-full border-collapse">
              ...
              <tbody>
                {students.map((s, idx) => (
                  <tr key={s.id} className={`${idx % 2 === 0 ? "bg-gray-50" : "bg-white"} hover:bg-green-50 transition`}>
                    ...
                    <td className="px-6 py-3">
                      <Button onClick={() => viewStudentDetails(s.id)} className="px-3 py-1 bg-blue-500 hover:bg-blue-600 text-white rounded-lg shadow-sm text-sm font-medium flex items-center gap-1">
                        <Eye className="w-4 h-4" /> View
                      </Button>
                    </td>
                    <td className="px-6 py-3 text-center">
                      <Button onClick={() => s.is_active ? deactivateStudent(s.id) : activateStudent(s.id)}
                        className={`px-4 py-2 rounded-lg text-sm font-medium shadow-sm ${s.is_active ? "bg-red-500 hover:bg-red-600" : "bg-green-500 hover:bg-green-600"} text-white`}
                      >
                        {s.is_active ? "Deactivate" : "Activate"}
                      </Button>
                    </td>
          ...
          {pagination && pagination.total_pages > 1 && (
            <div className="flex justify-center items-center gap-3 mt-10 flex-wrap">
              <Button disabled={page === 1} onClick={() => fetchStudents(page - 1, debouncedSearch)} className="p-2 rounded-full bg-gray-200 hover:bg-gray-300 disabled:opacity-50">
                <ChevronLeft className="w-5 h-5" />
              </Button>
              {Array.from({ length: pagination.total_pages }, (_, i) => i + 1).map(num => (
                <Button key={num} onClick={() => fetchStudents(num, debouncedSearch)} className={`${page === num ? "bg-green-900 text-white" : "bg-gray-100 text-gray-700 hover:bg-green-100"} px-4 py-2 rounded-full`}>
                  {num}
                </Button>
              ))}
              <Button disabled={page === pagination.total_pages} onClick={() => fetchStudents(page + 1, debouncedSearch)} className="p-2 rounded-full bg-gray-200 hover:bg-gray-300 disabled:opacity-50">
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold text-green-900">Student Details - {selectedStudent.name}</h2>
                <button onClick={() => setShowStudentDetails(false)} className="p-2 hover:bg-gray-100 rounded-full transition"><X className="w-6 h-6" /></button>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <div className="mt-1">
                      <span className={`px-3 py-1 text-sm font-semibold rounded-full ${selectedStudent.is_active ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}>
                  ...
                  <div>
                    ...
                    <div className="mt-1">
                      <span className={`px-3 py-1 text-sm font-semibold rounded-full ${selectedStudent.is_verified ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}>
                        {selectedStudent.is_verified ? "Verified" : "Unverified"}
                      </span>
                    </div>
                  </div>
                </div>
                      ...
                      <tbody>
                        {selectedStudent.course_enrollments.map((enrollment) => (
                          <tr key={enrollment.id} className="border-b hover:bg-gray-50">
                            ...
                            <td className="px-4 py-3">
                              <span className="px-2 py-1 text-xs font-semibold bg-blue-100 text-blue-700 rounded-full">{enrollment.course_category}</span>
                            </td>
                            <td className="px-4 py-3 text-sm text-gray-700">{new Date(enrollment.enrolled_at).toLocaleDateString()}</td>
                            <td className="px-4 py-3 text-sm text-gray-700">{enrollment.expires_at ? new Date(enrollment.expires_at).toLocaleDateString() : "No expiration"}</td>
                            <td className="px-4 py-3">
                              {enrollment.is_expired ? (
                                <span className="px-2 py-1 text-xs font-semibold bg-red-100 text-red-700 rounded-full">Expired</span>
                              ) : enrollment.is_active ? (
                                <span className="px-2 py-1 text-xs font-semibold bg-green-100 text-green-700 rounded-full">Active</span>
                              ) : (
                                <span className="px-2 py-1 text-xs font-semibold bg-gray-100 text-gray-700 rounded-full">Inactive</span>
                              )}
                            </td>
                            <td className="px-4 py-3 text-sm text-gray-700">{enrollment.days_remaining !== undefined ? (enrollment.days_remaining > 0 ? `${enrollment.days_remaining} days` : "Expired") : "N/A"}</td>
                            <td className="px-4 py-3">
                              <span className="px-2 py-1 text-xs font-semibold bg-purple-100 text-purple-700 rounded-full">{enrollment.enrollment_source}</span>
                            </td>
                          </tr>
                        ))}
                      </tbody>
