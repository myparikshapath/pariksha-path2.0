...
export default function AdminExamDetailPage({
  ...
  useEffect(() => {
    const initializeContent = async () => {
      ...
      try {
        ...
        const processedSections = (response.data.exam_info_sections || []).map(
          (s: any) => {
            let type: "text" | "table" = s.type;

            if (!type) {
              type = s.content?.startsWith("[") ? "table" : "text";
            }

            let tableData = [];
            if (type === "table") {
              try {
                tableData = JSON.parse(s.content || "[]");
              } catch {
                tableData = [];
              }
            }
        ...
        setExamContent({
          ...
          description: response.data.description || "",
          banner_url: response.data.banner_url || null,
          exam_info_sections: processedSections,
        });
      } catch (error) {
        setMode("add");
        setExamContent({
  ...
  const saveContent = async () => {
    ...
    const dataToSend = {
      ...
      description: examContent.description.trim(),
      banner_url: examContent.banner_url || null,
      exam_info_sections: examContent.exam_info_sections.map((s, index) => {
        if (s.type === "table") {
          return {
            ...s,
            order: index,
            is_active: true,
            content: JSON.stringify(s.tableData || []),
          };
        }
  ...
  return (
    <div className="max-w-5xl mx-auto p-6">
      <h1 className="text-3xl font-bold mb-6 text-green-900">
        {mode === "edit" ? "Edit" : "Add"} Content: {examContent.title}
      </h1>
      ...
      <input
        ...
        value={examContent.title}
        onChange={(e) =>
          setExamContent((prev) => ({ ...prev, title: e.target.value }))
        }
      />
                      ...
                      exam_info_sections: prev.exam_info_sections.map((s) =>
                        s.id === section.id
                          ? {
                            ...
                            type: "table",
                            tableData:
                              s.tableData && s.tableData.length > 0
                                ? s.tableData
                                : [{ col1: "", col2: "" }],
                ...
                {section.tableData?.map((row, index) => (
                  <div key={index} className="flex gap-2">
                    <input
                      ...
                      value={row.col1}
                      onChange={(e) => {
                        const newRow = [...(section.tableData || [])];
                        newRow[index].col1 = e.target.value;
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        const newRow = [...(section.tableData || [])];
                        newRow[index].col2 = e.target.value;
      ...
      <button
        onClick={saveContent}
        className="mt-8 bg-[#2E4A3C] text-white px-8 py-3 rounded"
      >
        Save Content
      </button>
    </div>
