...
//             { icon: BookOpen, name: "CTET / Teaching Exams", button: "Explore Now" },
//             { icon: BarChart2, name: "Competitive Exams", button: "Explore Now" },
//             { icon: Globe, name: "General Knowledge Quizzes", button: "Try Now" },
//           ].map((course, idx) => (
//             <Card
//               key={idx}
...
//             { number: 400, suffix: "+", label: "Courses Available" }, // New Field
//           ].map((stat, idx) => (
//             <Card
//               key={idx}
...
//                     </span>
//                   </div>
//                   <p className="text-[#2E4A3C] font-semibold text-lg">{item.name}</p>
//                   <p className="text-gray-700 text-sm mt-2">{item.text}</p>
//                 </CardContent>
//               </Card>
//             ))}
  ...
  return (
    <main className="pt-0">
      ...
      <section ref={statsRef} className="max-w-6xl mx-auto mt-16 text-center">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
          {[
            ...
            <Card key={idx} className="shadow-md rounded-xl hover:shadow-xl transition">
              <CardContent className="p-6">
                <p className="text-3xl sm:text-4xl font-extrabold bg-gradient-to-r from-[#2E4A3C] to-[#9BCB77] bg-clip-text text-transparent">
                  {statsInView ? (
                    <CountUp
                ...
                <p className="text-gray-700 text-sm sm:text-base font-medium">{stat.label}</p>
              </CardContent>
            </Card>
          ))}
        </div>
      ...
      <section className="max-w-6xl mx-auto mt-24 px-4 sm:px-0 flex flex-col md:flex-row gap-16 items-start mb-24">
        ...
        >
          <h2 className="text-2xl font-semibold text-[#2E4A3C] mb-8 text-center">Send Us a Message</h2>

          {[{ name: "name", label: "Full Name", type: "text" },
          { name: "email", label: "Email Address", type: "email" },
          { name: "phone", label: "Phone Number", type: "tel" }].map((field) => (
            <div key={field.name} className="relative mb-5">
              <input
                type={field.type}
                name={field.name}
        ...
        peer-focus:top-2 peer-focus:text-yellow-500 peer-focus:text-sm">
          ...
          <button
            type="submit"
            className="mt-8 w-full bg-yellow-400 hover:bg-yellow-500 text-black font-bold py-3 rounded-xl shadow-lg transition transform hover:-translate-y-1 hover:scale-105 cursor-pointer"
          >
            Send Message
          </button>
        </motion.form>
