...
export interface QuestionUpdateRequest {
  ...
  explanation?: string;
  explanation_images?: ImageAttachment[];
  remarks?: string;
  remarks_images?: ImageAttachment[];
  question_images?: ImageAttachment[];
  subject?: string;
  topic?: string;
  tags?: string[];
  is_active?: boolean;
...
): Promise<{ message: string; question_id: string; changes: QuestionChanges[] }> => {
  try {
    const response = await api.put(
...
export const deleteQuestion = async (
  questionId: string
): Promise<{ message: string; question_id: string; deleted_question: DeletedQuestion }> => {
  try {
